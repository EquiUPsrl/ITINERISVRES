...
arg_parser.add_argument('--modis_interval_8d', action='store', type=str, required=True, dest='modis_interval_8d')

arg_parser.add_argument('--oceancolor', action='store', type=str, required=True, dest='oceancolor')

arg_parser.add_argument('--resolution', action='store', type=str, required=True, dest='resolution')
...
modis_interval_8d = args.modis_interval_8d.replace('"','')
...
def generate_links(input_start_str, input_end_str, interval_type="8D", resolution="4km", product_type="CHL", variable="chlor_a"):
    ...
    if interval_type == "8D":
        intervals = load_and_filter_modis_intervals(modis_interval_8d, input_start, input_end) #generate_modis_8d_intervals(input_start, input_end)
...
for product_type in products_types:
    ...
    temp_path = os.path.join(conf_tmp_path, "nc", product_type)
    ...
    with open(os.path.join(conf_tmp_path, product_type + "_errors.txt") , "w", encoding="utf-8") as f:
        for err in errors:
            f.write(err + "\n")
