...
def extract_shapefiles_from_zip(input_dir, output_dir, include_associated=True):
    ...
    def extract_zip(zip_path, zip_nome_base, output_dir):
        """
        Extracts the file from a zip file and processes it recursively if it contains other zip files.
        Non-zip files are renamed to the name of the current zip file.
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for percorso in estratti:
                ...
                if ext == ".zip":
                    nome_zip_interno = os.path.splitext(os.path.basename(percorso))[0]
