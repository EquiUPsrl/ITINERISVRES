...
pearson_tmp_path = os.path.join(conf_tmp_path, "Pearson correlation coefficient")
os.makedirs(pearson_tmp_path, exist_ok=True)
...
for sub1, sub2 in combinations(subfolders, 2):
    ...
    output_file = os.path.join(pearson_tmp_path, out_name)
    df.to_csv(output_file, index=False)
    print(f"Table saved: {output_file}")

file_pearson_tmp_path = open("/tmp/pearson_tmp_path_" + id + ".json", "w")
file_pearson_tmp_path.write(json.dumps(pearson_tmp_path))
file_pearson_tmp_path.close()
