...
print("Verifico model_lm")
print(class(model_lm))
summary(model_lm)
...
if (exists("model_lm")) {
    tryCatch({
        ...
        write.csv(as.data.frame(importance_lm$importance), file = importance_file)
        
        cat("L'importanza delle variabili è stata salvata in:", importance_file, "\n")
    }, error = function(e) {
        cat("Errore durante il calcolo dell'importanza delle variabili:\n")
        print(e)
    })
} else {
    cat("Modello non definito, impossibile calcolare l'importanza delle variabili.\n")
}
