...
arg_parser.add_argument('--output_groupby_path', action='store', type=str, required=True, dest='output_groupby_path')
...
output_groupby_path = args.output_groupby_path.replace('"','')
...
filtered_input = output_groupby_path

if filtro :
    filtered_input = os.path.join(output_dir, 'filtered_input.csv')
    df = apply_csv_filter(output_groupby_path, filtro)
