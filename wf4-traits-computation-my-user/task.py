...
arg_parser.add_argument('--param_operator_file', action='store', type=str, required=True, dest='param_operator_file')
...
param_operator_file = args.param_operator_file.replace('"','')
...
def downloadFile(remote_url, local_path = "/tmp/data"):
...
datain = downloadFile(param_datain)
operator_file = downloadFile(param_operator_file)



output_file_traits = os.path.join(output_dir, 'test_advanced.csv')

df_datain = pd.read_csv(datain, sep=';', encoding='utf-8')
print("Colonne df_datain:", df_datain.columns.tolist())
...
calc_fun(datain, operator_file, CalcType, CompTraits, output_file_traits)
end_time = time.time()
print(f"Tempo di esecuzione: {end_time - start_time:.2f} secondi")

file_output_file_traits = open("/tmp/output_file_traits_" + id + ".json", "w")
file_output_file_traits.write(json.dumps(output_file_traits))
file_output_file_traits.close()
