...
library(reshape2)
mae_long <- melt(mae_df)

pairwise_results <- pairwise.wilcox.test(mae_long$value, mae_long$variable, p.adjust.method = "bonferroni")
...
output_file_path <- file.path(output_dir, "pairwise_comparison_results.txt")
write.table(pairwise_results$p.value, file = output_file_path, sep = "\t", row.names = TRUE, col.names = NA)

cat("Pairwise comparison results saved to:", output_file_path, "\n")
