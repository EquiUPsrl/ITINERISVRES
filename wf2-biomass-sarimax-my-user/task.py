...
input_base_folder = stats_output_path #os.path.join(work_path, "output", "Time Series Statistics", "corrected")
output_dir_base = os.path.join(output_dir, "SARIMAX")
os.makedirs(output_dir_base, exist_ok=True)


df = pd.read_csv(os.path.join(conf_input_path, "config_sarimax.csv"), sep=";", header=None, names=["key", "value"])

target_var = df.loc[df["key"] == "target_var", "value"].iloc[0]

exogenous_vars_str = df.loc[df["key"] == "exogenous_vars", "value"].iloc[0]
exogenous_vars = [v.strip() for v in exogenous_vars_str.split(",")]
forecast_time_in_years = int(df.loc[df["key"] == "forecast_time_in_years", "value"].iloc[0])
...
EXOG_FILE_SELECTION_MODE = "include_list" #"all_except_target"  # or "include_list"

EXOG_FILE_NAMES = [f"all_statistics_{var}.csv" for var in exogenous_vars]
...
def process_sarimax(target_file: str,
                    exog_files: list[str]):
    ...
    print("First future date:", future_index[0])
    print("Last future date:", future_index[-1])
    ...
    try:
        ...
        resid.to_csv(os.path.join(out_dir, f"Residuals_{ts_name}.csv"),
             sep=";", index=True, header=["residual"])
    ...
    for col in exog_cols:
        if not finite_mask[col].all():
            print(f"Column '{col}' contains non-finite or infinite values")
    ...
    print("NaN", X_future.isna().sum())      # conta NaN per colonna
...
ok, errs = 0, []
process_sarimax(target_file, exog_files)
ok += 1
