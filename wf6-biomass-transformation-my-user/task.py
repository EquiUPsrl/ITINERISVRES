...
parameter_file_path = parameters_csv
...
data_transformation_path = os.path.join(output_dir, "data_transformation.csv")
report_out_path = os.path.join(output_dir, "transformation_report.csv")

final_df.to_csv(data_transformation_path, index=False, encoding="utf-8-sig")
print(f"[OK] Transformed dataset saved in: {data_transformation_path}")

report_df = pd.DataFrame(report_rows)
report_df.to_csv(report_out_path, index=False, encoding="utf-8-sig")
print(f"[OK] Transformation report saved in: {report_out_path}")

file_data_transformation_path = open("/tmp/data_transformation_path_" + id + ".json", "w")
file_data_transformation_path.write(json.dumps(data_transformation_path))
file_data_transformation_path.close()
